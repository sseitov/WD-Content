...
int64_t shiftRingBufferTo(AudioRingBuffer* rb, int64_t pts)
{
	std::unique_lock<std::mutex> lock(rb->_mutex);
	int index = rb->_start;
	int64_t shift = AV_NOPTS_VALUE;
	while (index != rb->_end) {
		if (shift == AV_NOPTS_VALUE) {
			shift = pts - rb->_framePTS[index];
		}
		rb->_framePTS[index] += shift;
		index = (index + 1) % rb->_count;
	}
	return (rb->_start != rb->_end) ? rb->_framePTS[rb->_end] : AV_NOPTS_VALUE;
}

void shiftRingBufferNolock(AudioRingBuffer* rb, int64_t shift)
{
	int index = rb->_start;
	while (index != rb->_end) {
		rb->_framePTS[index] += shift;
