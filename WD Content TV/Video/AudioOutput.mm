...
- (int64_t)lock
{
	if (_ringBuffer)
	{
		_ringBuffer->_mutex.lock();
		return _ringBuffer->_currentPTS;
	}
...
- (void)unlock
{
	if (_ringBuffer)
	{
		_ringBuffer->_mutex.unlock();
		_ringBuffer->_overflow.notify_one();
	}
}
...
{
	if (_ringBuffer)
		return shiftRingBufferTo(_ringBuffer, pts);
	return AV_NOPTS_VALUE;
}
...
{
	if (_ringBuffer)
		shiftRingBufferNolock(_ringBuffer, shift);
}
...
- (void)pushPacket:(AVPacket*)packet
{
	...
	if (!_started) {
		...
		if (!success) {
			if ([self.delegate respondsToSelector:@selector(audioOutput:encounteredError:)]) {
				[self.delegate audioOutput:self encounteredError:[NSError errorWithDomain:@"com.vchannel.WD-Content" code:-1 userInfo:nil]];
			}
			return;
		}
	}
